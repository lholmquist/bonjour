...
const CircuitBreaker = require('brakes');
...
const circuit = new CircuitBreaker(roi.get, circuitOptions);
...
router.routes = (store) => {
  ...
  router.get('/bonjour-chaining', (req, resp) =>
    circuit.exec({endpoint: `http://${nextService}:8080/api/${nextService}-chaining`}).then((response) => {
      resp.set('Access-Control-Allow-Origin', '*');
      resp.type('application/json').send(response);
    }).catch((e) => resp.send(e))
  );

  const globalStats = circuit.getGlobalStats();
